...
*/
{
    // YOUR CODE HERE
  double x;
  int y = int_path_length(root,0);
  x = (double) y;
   
  return x/num_nodes;  // stub, remove after writing your code
}
...
int BinarySearchTree::int_path_length(BinaryNode *t, int depth) {
  ...
  if (t->left==NULL && t->right==NULL)
    ...
    x=depth+int_path_length(t->right,depth+1);
  else if (t->right==NULL)
    x=depth+int_path_length(t->left,depth+1);
  else
    x= depth+int_path_length(t->left,depth+1)+int_path_length(t->right,depth+1);
  
  return x; // remove after writing your code
}
